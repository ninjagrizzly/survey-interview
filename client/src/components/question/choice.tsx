...
const SingleChoice: FC<SingleChoiceProps> = memo(({data, onChange, initialValue = null}) => {
...
const MultiChoice: FC<MultiChoiceProps> = memo(({data, onChange, initialValue = []}) => {
    const [value, setValue] = useState(initialValue);
    const {label, options} = data;
    ...
    const addOption = useCallback((optionId: number) => {
                ...
                <FormGroup>
                    {options.map(({id, label}) => (
                        <FormControlLabel
                            key={id}
                            control={
                                <Checkbox key={id} checked={value.includes(id)} onChange={handleChange} value={id} />
                            }
                            label={label}
                        />
                    ))}
